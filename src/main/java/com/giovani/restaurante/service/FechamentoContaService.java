...
import com.giovani.restaurante.dto.FechamentoContaRequest;
import com.giovani.restaurante.dto.FechamentoContaResponse;
...
public class FechamentoContaService {
    ...
    public FechamentoContaResponse fecharConta(Long pedidoId, FechamentoContaRequest request) {
        ...
        if (fechamentoContaRepository.existsById(pedidoId)) {
        ...
        if (itens.isEmpty()) {
            throw new RegraNegocioException("Não pode fechar pedido sem item");
        }
        ...
        if (!itensNaoEntregue.isEmpty()) {
            throw new RegraNegocioException("Todos os itens precisam estar entregues");
        }
        ...
        if (taxaServico.compareTo(BigDecimal.ZERO) < 0) {
            throw new RegraNegocioException("Taxa de servico não pode ser negativo");
        }
